...
- **NFR9: Logging for traceability and troubleshooting purposes** — every attempt to use a coupon needs to be traceable end-to-end, not just the successful `coupon_usage` records, so incidents and abnormal patterns can be diagnosed after the fact. **Implemented** — Log4j2 (console only), a logger on every class with substantive business logic, business outcomes at INFO, denial reasons at WARN.
